...
| 命令 |	说明 |
| --- | --- |
| dev |	启动开发服务器 (8000端口)，监听文件变更，支持热重载。 |
| build |	将扩展打包导出到 dist/ 目录，生成最终的可发布文件。 |
| clean |	清理 dist/ 目录。 |
...
由于 TurboWarp 的安全策略，直接从 URL 加载的扩展会被强制放入沙箱中。
1. **特权加载**: 我们通过“文本加载”方式注入一个 **Loader (代理扩展)**。因为是从安全来源的代码，TurboWarp 允许其获得 **非沙箱权限**。
...
简单来说是推送两次扩展更新，第一次更新是把积木列表情况，这样会强制刷新积木。
