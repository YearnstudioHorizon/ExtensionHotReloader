...
* **🔥 实时热重载 (Hot Reload)**: 修改 `src` 目录下的代码并保存，TurboWarp 中的积木会自动更新，无需刷新页面。
* **⚡ 非沙箱环境 (Unsandboxed)**: 扩展运行在主线程，拥有完整的 `window` 和 DOM 访问权限。
...
3. 在 TurboWarp 中点击左下角的扩展图标。

4. 选择 **自定义扩展**。

5. 切换到 **文本** 选项卡。

6. 粘贴 (Ctrl+V) 代码并点击 加载 (Load)。

> ⚠️ 注意：请务必在弹出的安全警告中选择 "以非沙箱模式运行 (Run unsandboxed)"。
...
2. **实时监听**: Loader 运行后会持续轮询本地服务器。
3. **动态注入**: 监听到源码变更后，Loader 拉取新代码，通过 `eval` 在当前特权上下文中执行，并利用 `refreshBlocks` API 强制刷新积木视图。
